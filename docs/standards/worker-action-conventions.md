...
- `conversations.*` — операции над переписками (`v2` contract)
- `products.*` — операции над товарами (`v2` contract)
- `account.*` — операции чтения account metadata (`v2` contract)
...
- action-key для `v2` ресурсов (`account.*`, `conversations.*`, `products.*`) должен проксироваться в `/internal/v2/worker/*`;
...
- переименование `action` допускается только с документированным миграционным периодом;

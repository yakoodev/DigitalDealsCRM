...
- `CORE_SECRETS_ENCRYPTION_KEY` — ключ шифрования секретов интеграций в Core DB (project service tokens, Telegram proxy credentials);
- `FUNPAYSTAT_INTEGRATION_SERVICE_TOKEN` — service-auth токен DDCRM -> FunPayStat integration API;
- `STEAM_ACCOUNTS_MANAGER_INTEGRATION_SERVICE_TOKEN` — service-auth токен DDCRM -> Steam Accounts Manager integration API;
- `FUNPAYSTAT_PROJECT_TOKEN_SIGNING_SALT` — salt для валидации project token fingerprint в FunPayStat;
- `STEAM_ACCOUNTS_MANAGER_PROJECT_TOKEN_SIGNING_SALT` — salt для валидации project token fingerprint в Steam Accounts Manager;
...
- `FunPayStatClient:*`, `SteamAccountsManagerClient:*`, `EntitlementCheckClient:*`, `TelegramNotifications:*` — секции appsettings/ENV для integration bus runtime-клиентов и Telegram sender;

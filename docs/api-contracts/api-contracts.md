...
- `openapi-internal.yaml` покрывает service-to-service endpoint-ы (`/internal/...`), включая control-plane операции Accounts Manager (`worker-servers`, `heartbeat`, `lifecycle/rebalance`); все endpoint-ы требуют `service-auth` через `X-Service-Token` (security scheme `internalServiceToken`).

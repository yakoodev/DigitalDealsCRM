...
- `openapi-internal.yaml` покрывает service-to-service endpoint-ы (`/internal/...`); все endpoint-ы требуют `service-auth` через `X-Service-Token` (security scheme `internalServiceToken`).

# DDCRM — общий черновик архитектуры v2
...
- **Billing** — тарифы, add-on, платежи, подписки
- **Entitlement Service** — активные ограничения и разрешения проекта
...
- **Core DB** — бизнес-данные и маршрутные метаданные
...
sequenceDiagram
    ...
    BILL->>ENT: активировать доступы после оплаты
```
...
- при снижении тарифа или неоплате площадки **блокируются**, но не удаляются сразу
- Gateway перестаёт пускать в worker при блокировке проекта
- есть grace period
- есть ручные платёжные сценарии
- есть ручной override со стороны системного админа DDCRM
